...
play.py -> allows playing of the game and outputs the models chosen move in each round of a battle in the terminal
...
I really enjoyed having to deal with the emulators memory directly. I had to experiment with what data is where and build an understanding on how the game represented different values, for example finding a memory address to detect the timing of when to choose a move. This site was super helpful: https://datacrystal.tcrf.net/wiki/Pok%C3%A9mon_Red_and_Blue . Getting data out of the games ROM, say for move IDs to know which ones were problematic or type IDs, felt very sophisticated.
